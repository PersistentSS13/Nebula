FROM persistentss13/byond:512-latest as compile
COPY . /persistent
WORKDIR /persistent
RUN scripts/dm.sh nebula.dme


FROM persistentss13/byond:512-latest as test_setup
ENV LANG=C.UTF-8 \
...
RUN mkdir -p /persistent/data /persistent/config /persistent/nano
