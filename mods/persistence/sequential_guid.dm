...
/datum/uniqueness_generator/guid_generator/Generate(var/key, var/default_id = 100)
	...
	var/timeofday    = num2hex_padded(world.timeofday, 8)
	var/epoc         = num2hex_padded(world.realtime / 10, 8)
	var/variant      = num2hex_padded(min(variant_flag, 65535), 4)
	var/sequence     = num2hex_padded(sequence_id, 6)
