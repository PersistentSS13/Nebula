// Copy-paste override to save laces from a messy death.
// TO-DO: Generalize as a blacklist upstream
/obj/machinery/material_processing/compressor/Process()
	...
	if(input_turf)
		...
		for(var/obj/item/O in input_turf)
			if(!O.simulated || O.anchored || istype(O, /obj/item/organ/internal/stack))
				continue
		...
		if(emagged)
			for(var/mob/living/carbon/human/H in input_turf)
				for(var/obj/item/organ/external/crushing in H.get_organs())
