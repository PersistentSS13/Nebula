...
/obj/machinery/arrival_spawner
	...
	var/spawnpoint_type = /decl/spawnpoint/arrival_chargen

/obj/machinery/arrival_spawner/Initialize(mapload, d, populate_parts)
	if((. = ..()) == INITIALIZE_HINT_QDEL)
		return .
	var/obj/machinery/arrival_spawner/AS = locate(/obj/machinery/arrival_spawner) in loc
	if(AS && AS != src)
		log_warning("Existing arrival spawner located below [log_info_line(src)]. Deleting!")
		return INITIALIZE_HINT_QDEL
	return INITIALIZE_HINT_LATELOAD

/obj/machinery/arrival_spawner/LateInitialize()
	. = ..()
	var/decl/spawnpoint/SP = GET_DECL(spawnpoint_type)
	var/turf/T = get_turf(src)
	if(!T)
		CRASH("[log_info_line(src)] is on a null turf. Cannot be used as a spawn position!") //Will this crash the entire LateInitializeChain?
	SP.add_spawn_turf(T)
	events_repository.register(/decl/observ/moved, src, src, /obj/machinery/arrival_spawner/proc/on_moved)

/obj/machinery/arrival_spawner/Destroy()
	events_repository.unregister(/decl/observ/moved, src, src, /obj/machinery/arrival_spawner/proc/on_moved)
	var/decl/spawnpoint/SP = GET_DECL(spawnpoint_type)
	var/turf/T = get_turf(src)
	if(T)
...
/obj/machinery/arrival_spawner/proc/on_moved(atom/movable/moving_instance, atom/old_loc, atom/new_loc)
	var/decl/spawnpoint/SP = GET_DECL(spawnpoint_type)
	var/turf/T = get_turf(new_loc)
	if(!T)
		CRASH("[log_info_line(src)] is on a null turf. Cannot be used as a spawn position!")
	if(old_loc)
		SP.remove_spawn_turf(get_turf(old_loc))
	SP.add_spawn_turf(T)
