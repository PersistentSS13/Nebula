...
/datum/controller/subsystem/chargen/Initialize()
	...
	var/datum/map_load_metadata/M = maploader.load_map(file("maps/kleibkhar/chargen/chargen.dmm"), 1, 1, map_z)
	if (M)
		...
		for(var/x in 1 to Floor(world.maxx / width))
			for(var/y in 1 to Floor(world.maxy / height))
				...
				if(x == 1 && y == 1)
					continue
				maploader.load_map(file("maps/kleibkhar/chargen/chargen.dmm"), ((x - 1) * width) + 1, ((y - 1) * height) + 1, map_z, no_changeturf = TRUE)
				CHECK_TICK
