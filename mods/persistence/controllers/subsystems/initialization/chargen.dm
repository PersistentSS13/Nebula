...
/datum/controller/subsystem/chargen/proc/try_spawn_player_in_world(mob/living/player_mob, decl/spawnpoint/provider)
	//First make sure the player is set to have cleared completely chargen
	set_player_chargen_state(player_mob, CHARGEN_STATE_FINALIZED)
	//return provider.try_spawn(player_mob)
	//Check if we have an available spawn turf
	var/atom/spawnloc = SAFEPICK(provider.get_spawn_turfs(player_mob))
	if(!spawnloc)
...
///Sets the player's origin to the given id.
...
///Sets the player's role to the given id.
...
///Change the chargen state for a given player to the specified state. Ensures everything is updated together.
...
///Validates the picked origin and role set in their mind. The player has to use the chargen form to set those.
