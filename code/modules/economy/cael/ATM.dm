	...
	if(get_dist(src,user) <= 1)
		...
		if(emagged > 0)
			...
			if(ticks_left_locked_down > 0)
				...
				if(authenticated_account.suspended)
					...
					switch(view_screen)
						...
						if(TRANSFER_FUNDS)
							...
							t += "<input type='text' name='funds_amount' value='' style='width:200px; background-color:white; color:black;'><input type='submit' value='Withdraw'>"
							t += "</form>"
				...
				t += "<b>Account:</b> <input type='text' id='account_num' name='account_num' style='width:250px; background-color:white; color:black;'><BR><BR>"
				//Leave the PIN field out of sight until needed
				if(account_security_level)
					t += "<b>PIN:</b> <input type='text' id='account_pin' name='account_pin' style='width:250px; background-color:white; color:black;'><BR><BR>"
